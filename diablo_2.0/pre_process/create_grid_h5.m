...
% Modified by Vincent Laroche, 03/13/2025
...
% Specify the number of finite difference dimensions
disp('1) channel flow (finite difference in Y)');
disp('2) duct flow (finite difference in Y,Z)');
NUM_FD_DIR=input('Enter the number of finite difference dimensions: ');

% Look through each dimension
for i=1:NUM_FD_DIR
    disp(' ')
    disp(['*** Creating grid for finite difference dimension ' num2str(i) ' ***'])
    ...
    % Set the dimensions of the grid 
     N=input('Enter the grid size (this should match grid_def.all): ');
    ...
    elseif i==2
        h5create('grid.h5','/grids/z',N+1)
        h5write ('grid.h5','/grids/z',G(2:N+2))
    end
%     clear G GF
end
